...
import { View, Text, ImageBackground, StyleSheet, TouchableOpacity, TextInput, ActivityIndicator, Alert } from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';

const loginscreen = require('../assets/loginscreen1.png');

export const RefLogin = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        navigation.navigate('LandingScreen'); // Navigate to the next screen
      } else {
  ...
  return (
    <ImageBackground source={loginscreen} style={styles.backgrounddsa}>
      <View style={styles.containerdsa}>
        <Text style={styles.dsalogintext}>𝗥𝗲𝗳 𝗟𝗼𝗴𝗶𝗻</Text>
